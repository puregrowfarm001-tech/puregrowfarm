...
async function saveAdminExpense(e) {
  ...
  const categoryVal = document.getElementById("expCategory").value;
  const payerVal = document.getElementById("expPayer").value;
  const modeVal = document.getElementById("expMode").value;
  const descVal = document.getElementById("expDesc").value.trim();
  const notesVal = document.getElementById("expNotes") ? document.getElementById("expNotes").value.trim() : "";

  if (isNaN(amountVal) || amountVal <= 0) {
    alert("⚠️ Kripya valid amount dalein!");
    return;
  }
  ...
  // Supabase cloud me insert karein
